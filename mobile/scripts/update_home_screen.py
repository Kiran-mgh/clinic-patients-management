            ...
            <ScrollView style={{ flex: 1 }} contentContainerStyle={{ paddingVertical: 12 }}>
              ...
              >
                ...
                <View style={{ flex: 1 }}>
                  ...
                  <Text style={styles.menuItemSub}>Payments, receipt notes & balance</Text>
                </View>
          ...
          <View style={styles.modalContent}>
            ...
            {loadingHistory ? (
              <View style={{ padding: 40, alignItems: 'center' }}>
                <ActivityIndicator size="large" color="#213932" />
                <Text style={{ color: '#64748b', marginTop: 12 }}>Loading ledger history...</Text>
              </View>
            ) : (
              <ScrollView contentContainerStyle={{ gap: 14, paddingBottom: 20 }}>
                <View style={styles.ledgerSummaryCard}>
                  <View style={styles.ledgerStatBox}>
                    <Text style={styles.ledgerStatLabel}>TOTAL VISITS</Text>
                    <Text style={styles.ledgerStatVal}>{patientHistory.length}</Text>
                  </View>
                  <View style={[styles.ledgerStatBox, { borderLeftWidth: 1, borderRightWidth: 1, borderColor: '#e2e8f0' }]}>
                    <Text style={styles.ledgerStatLabel}>PAID TOKENS</Text>
                    <Text style={[styles.ledgerStatVal, { color: '#16a34a' }]}>
                      {patientHistory.filter((t: any) => t.paymentStatus === 'paid').length}
                    </Text>
                  </View>
                  <View style={styles.ledgerStatBox}>
                    <Text style={styles.ledgerStatLabel}>UNPAID TOKENS</Text>
                    <Text style={[styles.ledgerStatVal, { color: '#dc2626' }]}>
                      {patientHistory.filter((t: any) => t.paymentStatus !== 'paid' && t.status === 'served').length}
                    </Text>
                  </View>
                </View>

                <Text style={{ fontSize: 13, fontWeight: '800', color: '#1e293b', marginTop: 6 }}>ITEMIZED VISIT TRANSACTIONS</Text>

                {patientHistory.length === 0 ? (
                  <View style={{ padding: 30, alignItems: 'center', backgroundColor: '#f8fafc', borderRadius: 12 }}>
                    <Text style={{ fontSize: 32 }}>📜</Text>
                    <Text style={{ color: '#64748b', marginTop: 8, fontWeight: '600' }}>No visit ledger records found yet.</Text>
                  </View>
                ) : (
                    ...
                    <View key={item.id} style={styles.ledgerItemCard}>
                      <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
                        <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}>
                          <Text style={styles.ledgerTokenNum}>{item.tokenNumber}</Text>
                          <Text style={styles.ledgerServiceBadge}>
                            {item.serviceType === 'medicine' ? '💊 Medicine' : '🩺 Treatment'}
                          </Text>
                        </View>
                        <Text style={[
                          styles.paymentBadge,
                          item.paymentStatus === 'paid' ? styles.paymentPaid : styles.paymentUnpaid
                        ]}>
                      ...
                      <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginTop: 8 }}>
                        <Text style={styles.ledgerDateText}>🗓️ {formatToIndianDate(item.generatedAt)}</Text>
                        <Text style={[styles.statusBadgeSmall, (styles as any)[`status_${item.status}`] || styles.status_waiting]}>
                          {item.status.toUpperCase()}
                        </Text>
                      </View>

                      {item.paymentNotes ? (
                        <View style={styles.transactionNoteBox}>
                          <Text style={styles.transactionNoteText}>💳 Txn Note: {item.paymentNotes}</Text>
                        </View>
                      ) : null}

                      {item.notes ? (
                        <View style={styles.doctorNoteBox}>
                          <Text style={styles.doctorNoteText}>📝 Doctor Note: {item.notes}</Text>
                        </View>
                      ) : null}
                    </View>
                  ))
